...
namespace Portal.Core
{
    public class AccountDashboardBL
    {
        ...
        public List<BookBalanceViewModel> GetBookBalanceList(int companyId,int finYearId,out string bookList,out string balanceList)
        {
            ...
            try
            {
                DataTable dtbookBalance = sqlDbInterface.GetDashboard_BookBalance(companyId,finYearId);
        ...
        public List<MonthWiseCashBankTrnSummaryViewModel> GetMonthWiseBankCashTrnList(int companyId, int finYearId, out string monthList, out string bankPaymentList,out string bankReceiptList,out string cashPaymentList,out string cashReceiptList)
        {
            List<MonthWiseCashBankTrnSummaryViewModel> trnList = new List<MonthWiseCashBankTrnSummaryViewModel>();
            SQLDbInterface sqlDbInterface = new SQLDbInterface();
            try
            {
                DataTable dtbookBalance = sqlDbInterface.GetDashboard_MonthWiseBankCashTransactionSummary(companyId, finYearId);
                if (dtbookBalance != null && dtbookBalance.Rows.Count > 0)
                {
                    foreach (DataRow dr in dtbookBalance.Rows)
                    {
