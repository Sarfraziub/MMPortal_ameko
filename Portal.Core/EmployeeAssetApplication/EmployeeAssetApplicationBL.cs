...
namespace Portal.Core
{
    public class EmployeeAssetApplicationBL
    {
        ...
        public List<EmployeeAssetApplicationViewModel> GetEmployeeAssetApplicationList(string applicationNo , int employeeId , string assetTypeName, string assetStatus , string fromDate , string toDate , int companyId)
        {
            List<EmployeeAssetApplicationViewModel> employeeAssetApplicationViewModel = new List<EmployeeAssetApplicationViewModel>();          
            HRSQLDBInterface hRSQLDBInterface = new HRSQLDBInterface();
            try
            {
                DataTable dtAssetTypeApplication = hRSQLDBInterface.GetEmployeeAssetApplicationList(applicationNo, employeeId, assetTypeName, assetStatus, Convert.ToDateTime(fromDate), Convert.ToDateTime(toDate),companyId);
        ...
        public List<EmployeeAssetApplicationViewModel> GetEmployeeAssetApplicationApprovalList(string applicationNo, int employeeId, string assetTypeName, string assetStatus, string fromDate, string toDate, int companyId)
        {
            List<EmployeeAssetApplicationViewModel> employeeAssetApplicationViewModel = new List<EmployeeAssetApplicationViewModel>();
            HRSQLDBInterface hRSQLDBInterface = new HRSQLDBInterface();
            try
            {
                DataTable dtAssetTypeApplication = hRSQLDBInterface.GetEmployeeAssetApplicationApprovalList(applicationNo, employeeId, assetTypeName, assetStatus, Convert.ToDateTime(fromDate), Convert.ToDateTime(toDate), companyId);
                if (dtAssetTypeApplication != null && dtAssetTypeApplication.Rows.Count > 0)
                {
                    foreach (DataRow dr in dtAssetTypeApplication.Rows)
                    {
