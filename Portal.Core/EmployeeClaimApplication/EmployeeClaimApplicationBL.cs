...
namespace Portal.Core
{
    public class EmployeeClaimApplicationBL
    {
        ...
        public List<EmployeeClaimApplicationViewModel> GetEmployeeClaimApplicationList(string applicationNo , int employeeId , int ClaimTypeId, string claimStatus , string fromDate , string toDate , int companyId)
        {
            List<EmployeeClaimApplicationViewModel> employeeClaimApplicationViewModel = new List<EmployeeClaimApplicationViewModel>();          
            HRSQLDBInterface hRSQLDBInterface = new HRSQLDBInterface();
            try
            {
                DataTable dtClaimTypeApplication = hRSQLDBInterface.GetEmployeeClaimApplicationList(applicationNo, employeeId, ClaimTypeId, claimStatus, Convert.ToDateTime(fromDate), Convert.ToDateTime(toDate),companyId);
        ...
        public List<EmployeeClaimApplicationViewModel> GetEmployeeClaimApplicationApprovalList(string applicationNo, int employeeId, int claimTypeId, string claimStatus, string fromDate, string toDate, int companyId)
        {
            List<EmployeeClaimApplicationViewModel> employeeClaimApplicationViewModel = new List<EmployeeClaimApplicationViewModel>();
            HRSQLDBInterface hRSQLDBInterface = new HRSQLDBInterface();
            try
            {
                DataTable dtClaimTypeApplication = hRSQLDBInterface.GetEmployeeClaimApplicationApprovalList(applicationNo, employeeId, claimTypeId, claimStatus, Convert.ToDateTime(fromDate), Convert.ToDateTime(toDate), companyId);
                if (dtClaimTypeApplication != null && dtClaimTypeApplication.Rows.Count > 0)
                {
                    foreach (DataRow dr in dtClaimTypeApplication.Rows)
                    {
