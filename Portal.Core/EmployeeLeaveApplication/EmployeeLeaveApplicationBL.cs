...
namespace Portal.Core
{
    public class EmployeeLeaveApplicationBL
    {
        ...
        public List<EmployeeLeaveApplicationViewModel> GetEmployeeLeaveApplicationList(string applicationNo, int employeeId, string leaveTypeId, string leaveStatus, string fromDate, string toDate, int companyId)
        {
            List<EmployeeLeaveApplicationViewModel> employeeleaveApplications = new List<EmployeeLeaveApplicationViewModel>();
            HRSQLDBInterface sqlDbInterface = new HRSQLDBInterface();
            try
            {
                DataTable dtEmployeeLeaveApplications = sqlDbInterface.GetEmployeeLeaveApplicationList(applicationNo, employeeId, leaveTypeId, leaveStatus, Convert.ToDateTime(fromDate), Convert.ToDateTime(toDate), companyId);
        ...
        public List<EmployeeLeaveApplicationViewModel> GetEmployeeLeaveApplicationApprovalList(string applicationNo, int employeeId, string leaveTypeId, string leaveStatus, string fromDate, string toDate, int companyId)
        {
            List<EmployeeLeaveApplicationViewModel> employeeLeaveApplications = new List<EmployeeLeaveApplicationViewModel>();
            HRSQLDBInterface sqlDbInterface = new HRSQLDBInterface();
            try
            {
                DataTable dtEmployeeLeaveApplications = sqlDbInterface.GetEmployeeLeaveApplicationApprovalList(applicationNo, employeeId, leaveTypeId, leaveStatus, Convert.ToDateTime(fromDate), Convert.ToDateTime(toDate), companyId);
                if (dtEmployeeLeaveApplications != null && dtEmployeeLeaveApplications.Rows.Count > 0)
                {
                    foreach (DataRow dr in dtEmployeeLeaveApplications.Rows)
                    {
