...
namespace Portal.Core
{
    public class GeneralLedgerWOFYPrintBL
    {
        ...
        public ResponseOut GenerateGeneralLedger(int glId, int companyId, DateTime fromDate, DateTime toDate, int reportUserId, string sessionId)
        {
            ResponseOut responseOut = new ResponseOut();
            SQLDbInterface sqlDbInterface = new SQLDbInterface();
            try
            {
                responseOut = sqlDbInterface.GenerateGeneralLedgerWithoutFinalcialYear(glId, companyId,  fromDate,  toDate, reportUserId, sessionId);
            }
        ...
        public DataTable GetGeneralLedgerDetailDataTable(int reportUserId, string sessionId)
        {
            SQLDbInterface sqlDbInterface = new SQLDbInterface();
            DataTable dtGeneralLedger = new DataTable();
            try
            {
                dtGeneralLedger = sqlDbInterface.GetGeneralLedgerWithoutFinancialYearDetail(reportUserId,sessionId);
            }
            catch (Exception ex)
            {
                Logger.SaveErrorLog(this.ToString(), MethodBase.GetCurrentMethod().Name, ex);
