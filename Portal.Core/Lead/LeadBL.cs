...
namespace Portal.Core
{
    public class LeadBL
    {
        ...
        public List<LeadViewModel> GetLeadList(string leadCode = "", string companyName = "", string contactPersonName = "",  string email = "", string contactNo = "", string companyAddress = "", string companyCity = "", int companyStateId = 0,  int leadStatusId = 0, int leadSourceId = 0,int userId=0 ,string status = "", int reportingUserId=0, int reportingRoleId=0)
        {
            List<LeadViewModel> leads = new List<LeadViewModel>();
            SQLDbInterface sqlDbInterface = new SQLDbInterface();
            try
            {
                DataTable dtLeads = sqlDbInterface.GetLeadList(leadCode, companyName, contactPersonName, email,  contactNo, companyAddress, companyCity, companyStateId, leadStatusId, leadSourceId, userId, status,reportingUserId,reportingRoleId);
        ...
        public DataTable GenerateLeadReport(string leadCode = "", string companyName = "", string contactPersonName = "", string email = "", string contactNo = "", string companyAddress = "", string companyCity = "", int companyStateId = 0, int leadStatusId = 0, int leadSourceId = 0, int userId = 0, string status = "", int reportingUserId = 0, int reportingRoleId = 0)
        {
            SQLDbInterface sqlDbInterface = new SQLDbInterface();
            DataTable dtLead = new DataTable();
            try
            {
                dtLead = sqlDbInterface.GenerateLeadReport(leadCode, companyName, contactPersonName, email, contactNo, companyAddress, companyCity, companyStateId, leadStatusId, leadSourceId, userId, status, reportingUserId, reportingRoleId);
            }
            catch (Exception ex)
            {
                Logger.SaveErrorLog(this.ToString(), MethodBase.GetCurrentMethod().Name, ex);
