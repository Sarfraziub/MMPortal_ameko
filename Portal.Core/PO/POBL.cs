 ...
 public class POBL
    {
        ...
        public List<PurchaseInvoiceProductDetailViewModel> GetPOProductListForMRN(long InvoiceId)
        {
            List<PurchaseInvoiceProductDetailViewModel> piProducts = new List<PurchaseInvoiceProductDetailViewModel>();
            SQLDbInterface sqlDbInterface = new SQLDbInterface();
            try
            {
                DataTable dtProducts = sqlDbInterface.GetPOProductListForMRN(InvoiceId);
                if (dtProducts != null && dtProducts.Rows.Count > 0)
                {
                    foreach (DataRow dr in dtProducts.Rows)
                    {
                        ...
                        {
                            InvoiceProductDetailId = Convert.ToInt32(dr["POProductDetailId"]),
                            SequenceNo = Convert.ToInt32(dr["SNo"]),
                            ProductId = Convert.ToInt32(dr["ProductId"]),
                            ProductName = Convert.ToString(dr["ProductName"]),
                            ProductCode = Convert.ToString(dr["ProductCode"]),
        ...
        public ResponseOut AddEditPO(POViewModel poViewModel, List<POProductViewModel> poProducts, List<POTaxViewModel> poTaxes, List<POTermViewModel> poTerms, List<POSupportingDocumentViewModel> poDocuments)
        {
            ...
            try
            {
                PO po = new PO
                {
                    ...
                    CustomerBranchId = poViewModel.CustomerBranchId,
        ...
        public List<POViewModel> GetPOListForMRN(string poNo, string vendorName, string refNo, string fromDate, string toDate, string approvalStatus, int companyId, string displayType = "")
        {
            List<POViewModel> pos = new List<POViewModel>();
            SQLDbInterface sqlDbInterface = new SQLDbInterface();
            try
            {
                DataTable dtPOs = sqlDbInterface.GetPOListForMRN(poNo, vendorName, refNo, Convert.ToDateTime(fromDate), Convert.ToDateTime(toDate), approvalStatus, companyId, displayType);
                if (dtPOs != null && dtPOs.Rows.Count > 0)
                {
                    foreach (DataRow dr in dtPOs.Rows)
                    {
        ...
        public List<POViewModel> GetPOLessThan25KList(string poNo, string vendorName, string refNo, string fromDate, string toDate, string approvalStatus, int companyId, string displayType = "")
        {
            List<POViewModel> pos = new List<POViewModel>();
            SQLDbInterface sqlDbInterface = new SQLDbInterface();
            try
            {
                DataTable dtPOs = sqlDbInterface.GetPOLessThan25KList(poNo, vendorName, refNo, Convert.ToDateTime(fromDate), Convert.ToDateTime(toDate), approvalStatus, companyId, displayType);
                if (dtPOs != null && dtPOs.Rows.Count > 0)
                {
                    foreach (DataRow dr in dtPOs.Rows)
                    {
        ...
        public List<PurchaseQuotationViewModel> GetPurchaseQuotationList(string quotationNo, string vendorName, string fromDate, string toDate, int companyId, string displayType = "", string approvalStatus = "")
        {
            List<PurchaseQuotationViewModel> quotations = new List<PurchaseQuotationViewModel>();
            SQLDbInterface sqlDbInterface = new SQLDbInterface();
            try
            {
                DataTable dtQuotations = sqlDbInterface.GetPOPurchaseQuotationList(quotationNo, vendorName, Convert.ToDateTime(fromDate), Convert.ToDateTime(toDate), companyId, displayType, approvalStatus);
        ...
        public List<POEscalationViewModel> GetPOEscalationList(int companyId, int finYearId)
        {
            List<POEscalationViewModel> escalatePOs = new List<POEscalationViewModel>();
            SQLDbInterface sqlDbInterface = new SQLDbInterface();
            try
            {
                DataTable dtPOs = sqlDbInterface.GetPOEscalationList(companyId, finYearId);
                if (dtPOs != null && dtPOs.Rows.Count > 0)
                {
                    foreach (DataRow dr in dtPOs.Rows)
                    {
                        escalatePOs.Add(new POEscalationViewModel
                        {
                            PONo = Convert.ToString(dr["PONo"]),
                            POStatus = string.IsNullOrEmpty(Convert.ToString(dr["POStatus"])) ? "" : Convert.ToString(dr["POStatus"]),
                            PODate = Convert.ToString(dr["PODate"]),
        ...
        public List<POEscalationViewModel> GetPOLessThan25KEscalationList(int companyId, int finYearId)
        {
            List<POEscalationViewModel> escalatePOs = new List<POEscalationViewModel>();
            SQLDbInterface sqlDbInterface = new SQLDbInterface();
            try
            {
                DataTable dtPOs = sqlDbInterface.GetPOLessThan25KEscalationList(companyId, finYearId);
                if (dtPOs != null && dtPOs.Rows.Count > 0)
                {
                    foreach (DataRow dr in dtPOs.Rows)
                    {
        ...
        public List<POViewModel> GetApprovePOList(string poNo, string vendorName, string refNo, string fromDate, string toDate, string approvalStatus, int companyId, string displayType = "")
        {
            List<POViewModel> pos = new List<POViewModel>();
            SQLDbInterface sqlDbInterface = new SQLDbInterface();
            try
            {
                DataTable dtPOs = sqlDbInterface.GetApprovelPOList(poNo, vendorName, refNo, Convert.ToDateTime(fromDate), Convert.ToDateTime(toDate), approvalStatus, companyId, displayType);
                if (dtPOs != null && dtPOs.Rows.Count > 0)
                {
                    foreach (DataRow dr in dtPOs.Rows)
                    {
        ...
        public DataTable GetApprovedPOListByMD(DateTime fromDate, DateTime toDate, string approvalStatus, int companyId, int customerId, int customerBranchId, string displayType = "")
        {
            ...
            try
            {
                dtPOs = sqlDbInterface.GetApprovedPOListByMD(fromDate,toDate,approvalStatus,companyId,customerId,customerBranchId,displayType);
            }
            catch (Exception ex)
            {
                Logger.SaveErrorLog(this.ToString(), MethodBase.GetCurrentMethod().Name, ex);
        ...
        public List<POViewModel> GetDashboardRecommendationPOList(int companyId)
        {
            List<POViewModel> poList = new List<POViewModel>();
            SQLDbInterface sqlDbInterface = new SQLDbInterface();
            try
            {
                DataTable dtCount = sqlDbInterface.GetDashboardRecommendationPOList(companyId);
