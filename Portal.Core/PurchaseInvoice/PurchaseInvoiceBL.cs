...
namespace Portal.Core
{
    public class PurchaseInvoiceBL
    {
        ...
        public ResponseOut AddEditPI(PurchaseInvoiceViewModel piViewModel, List<PurchaseInvoiceProductDetailViewModel> piProducts, List<PurchaseInvoiceTaxDetailViewModel> piTaxes, List<PurchaseInvoiceTermsDetailViewModel> piTerms, List<PISupportingDocumentViewModel> piDocuments)
        {
            ...
            try
            {
                PurchaseInvoice pi = new PurchaseInvoice
                {
                    ...
                    CustomerBranchId = piViewModel.CustomerBranchId,
                ...
                responseOut = sqlDbInterface.AddEditPI(pi, piProductList, piTaxList, piTermList, piDocumentList);
            }
        ...
        public List<POViewModel> GetPOListForPI(string poNo, string vendorName, string refNo, string fromDate, string toDate, string approvalStatus, int companyId, string displayType = "")
        {
            List<POViewModel> pos = new List<POViewModel>();
            SQLDbInterface sqlDbInterface = new SQLDbInterface();
            try
            {
                DataTable dtPOs = sqlDbInterface.GetPOListForPI(poNo, vendorName, refNo, Convert.ToDateTime(fromDate), Convert.ToDateTime(toDate), approvalStatus, companyId, displayType);
                if (dtPOs != null && dtPOs.Rows.Count > 0)
                {
                    foreach (DataRow dr in dtPOs.Rows)
                    {
