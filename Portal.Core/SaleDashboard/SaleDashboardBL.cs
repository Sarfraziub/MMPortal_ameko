...
namespace Portal.Core
{
    public class SaleDashboardBL
    {
        ...
        public List<SOCountViewModel> GetSOCountList(int companyId, int userId, int reportingUserId, int reportingRoleId, int finYearId, out string sOCountList, out string sOtotalCountList)
        {
            ...
            try
            {
                DataTable dtquatationCount = sqlDbInterface.GetDashboard_SaleOrderCount(companyId, userId, reportingUserId, reportingRoleId, finYearId);
                if (dtquatationCount != null && dtquatationCount.Rows.Count > 0)
                {
                    foreach (DataRow dr in dtquatationCount.Rows)
                    {
        ...
        public List<SICountViewModel> GetSICountList(int companyId, int userId, int reportingUserId, int reportingRoleId, int finYearId, out string sICountList, out string sItotalCountList)
        {
            ...
            try
            {
                DataTable dtquatationCount = sqlDbInterface.GetDashboard_SICount(companyId,userId,reportingUserId,reportingRoleId, finYearId);
                if (dtquatationCount != null && dtquatationCount.Rows.Count > 0)
                {
                    foreach (DataRow dr in dtquatationCount.Rows)
                    {
