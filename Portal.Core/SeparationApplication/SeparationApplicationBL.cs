...
namespace Portal.Core
{
    public class SeparationApplicationBL
    {
        ...
        public List<SeparationApplicationViewModel> GetSeparationApplicationList(string applicationNo, int employeeId , string separationcategoryId,  string applicationStatus, string fromDate, string toDate, int companyId)
        {
            List<SeparationApplicationViewModel> separationApplications = new List<SeparationApplicationViewModel>();
            HRSQLDBInterface sqlDbInterface = new HRSQLDBInterface();
            try
            {
                DataTable dtSeparationApplications = sqlDbInterface.GetSeparationApplicationList(applicationNo, employeeId, separationcategoryId, applicationStatus, Convert.ToDateTime(fromDate), Convert.ToDateTime(toDate), companyId);
        ...
        public List<SeparationApplicationViewModel> GetSeparationApplicationApprovalList(string applicationNo, int employeeId, string separationcategoryId, string applicationStatus, string fromDate, string toDate, int companyId)
        {
            List<SeparationApplicationViewModel> separationApplications = new List<SeparationApplicationViewModel>();
            HRSQLDBInterface sqlDbInterface = new HRSQLDBInterface();
            try
            {
                DataTable dtSeparationApplications = sqlDbInterface.GetSeparationApplicationApprovalList(applicationNo, employeeId, separationcategoryId, applicationStatus, Convert.ToDateTime(fromDate), Convert.ToDateTime(toDate), companyId);
                if (dtSeparationApplications != null && dtSeparationApplications.Rows.Count > 0)
                {
                    foreach (DataRow dr in dtSeparationApplications.Rows)
                    {
