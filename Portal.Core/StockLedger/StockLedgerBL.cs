...
namespace Portal.Core
{
    public class StockLedgerBL
    {
        ...
        public decimal GetSiteProductAvailableStock(long productId, int finYearId, int customerId, int customerBranchId)
        {
            ...
            try
            {
                availableStock = sqlDbInterface.GetSiteProductAvailableStock(productId, finYearId, customerId, customerBranchId);
            }
            catch (Exception ex)
            {
                Logger.SaveErrorLog(this.ToString(), MethodBase.GetCurrentMethod().Name, ex);
                ...
                if (dtStockLedgers != null && dtStockLedgers.Rows.Count > 0)
                {
                    foreach (DataRow dr in dtStockLedgers.Rows)
                    {
                        stockLedgerViewModel.Add(new StockLedgerViewModel
                        {
                            ...
                            SaleQty = Convert.ToDecimal(dr["SaleQty"]),
                            StockInQty = Convert.ToDecimal(dr["STNQty"]),
                            StockOutQty = Convert.ToDecimal(dr["STRQty"]),
        ...
        public List<StockLedgerViewModel> GetStockLedgerDrilDownList(long productId, DateTime fromDate, DateTime toDate, int companyId)
        {
            List<StockLedgerViewModel> stockLedgers = new List<StockLedgerViewModel>();
            SQLDbInterface sqlDbInterface = new SQLDbInterface();
            try
            {
                DataTable dtProducts = sqlDbInterface.GetStockLedgerDrilDownList(productId, fromDate, toDate, companyId);
