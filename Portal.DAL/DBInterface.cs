...
namespace Portal.DAL
{
    ...
    public partial class DBInterface : IDisposable
    {
            ...
            try
            {
                ...
                else
                {
                    if (comapnyBranch.CompanyBranchId == 0)
                    {
                        ...
                        else if (entities.ComapnyBranches.Any(x => x.GSTNo == comapnyBranch.GSTNo && x.BranchName == comapnyBranch.BranchName && x.CompanyBranchId != comapnyBranch.CompanyBranchId && x.StateId != comapnyBranch.StateId))
                        {
                            responseOut.status = ActionStatus.Fail;
                            responseOut.message = ActionMessage.CompanyBranchDuplicateGSTNo;
                        }
        ...
        public List<Product> GetProductAutoCompleteListByMainGroup(string searchTerm, int productMainGroupId, int companyId)
        {
            List<Product> productList = new List<Product>();
            try
            {
                var products = (from p in entities.Products
                                where (p.ProductName.ToLower().Contains(searchTerm.ToLower()) || p.ProductCode.ToLower().Contains(searchTerm.ToLower()) || p.ProductShortDesc.ToLower().Contains(searchTerm.ToLower())) && p.ProductMainGroupId == productMainGroupId && p.CompanyId == companyId && p.Status == true
                                select new
                                {
                                    ProductId = p.Productid,
                                    ProductName = p.ProductName,
        ...
        public decimal GetProductPurchasePrice(int productId)
        {
            ...
            try
            {
                PurchasePrice = Convert.ToDecimal(entities.Products.Where(e => e.Productid == productId).Select(v => v.PurchasePrice).FirstOrDefault());
            }
            catch (Exception ex)
            {
                Logger.SaveErrorLog(this.ToString(), MethodBase.GetCurrentMethod().Name, ex);
        ...
        public ResponseOut AddEditLead(Lead lead, out int leadId)
        {
            ...
            try
            {
                ...
                else if (entities.Leads.Any(x => x.CompanyName == lead.CompanyName && x.ContactNo == lead.ContactNo && x.CompanyId == lead.CompanyId && x.LeadStatusId != 4 && x.LeadId != lead.LeadId))
                {
                    responseOut.status = ActionStatus.Fail;
        ...
        public List<Employee> GetTempEmployeeAutoCompleteList(string searchTerm, int departmentId, int designationId)
        {
            List<Employee> employeeList = new List<Employee>();
            try
            {
                var employees = (from emp in entities.Employees
                                 where ((emp.FirstName.ToLower().Contains(searchTerm.ToLower()) || emp.EmployeeCode.ToLower().Contains(searchTerm.ToLower()) || emp.MobileNo.Contains(searchTerm)) && emp.DepartmentId == (departmentId == 0 ? emp.DepartmentId : departmentId) && emp.DesignationId == (designationId == 0 ? emp.DesignationId : designationId) && emp.Status == true && emp.EmploymentType == "Temporary")
                                 select new
                                 {
                                     EmployeeId = emp.EmployeeId,
                                     FirstName = emp.FirstName + " " + emp.LastName,
        ...
        public List<CustomerBranch> GetCustomerBranchList(int customerId)
        {
            ...
            try
            {
                ...
                if (customerBranches != null && customerBranches.Count > 0)
                {
                    ...
                    {
                        customerList.Add(new CustomerBranch { CustomerBranchId = item.CustomerBranchId, BranchName = item.BranchName, GSTNo = item.GSTNo });
        ...
        public List<CustomerBranch> GetCustomerBranchListWithOutCustId()
        {
            List<CustomerBranch> customerList = new List<CustomerBranch>();
            try
            {
                var customerBranches = entities.CustomerBranches.Where(x => x.Status == true).Select(s => new
                {
                    CustomerBranchId = s.CustomerBranchId,
                    BranchName = s.BranchName,
                    GSTNo = s.GSTNo
        ...
        public ResponseOut AddEditProductGLMapping(ProductGLMapping productGLMapping)
        {
            ...
            try
            {
                if (entities.ProductGLMappings.Any(x => x.ProductSubGroupId == productGLMapping.ProductSubGroupId && x.MappingId != productGLMapping.MappingId))
                {
                    responseOut.status = ActionStatus.Fail;
        ...
        public long GetIdBySLHead(string sLHead)
        {
            ...
            try
            {
                sLId = entities.SLs.Where(s => s.SLCode.Trim().ToUpper() == sLHead.Trim().ToUpper()).Select(x => x.SLId).FirstOrDefault();
            }
            catch (Exception ex)
            {
                Logger.SaveErrorLog(this.ToString(), MethodBase.GetCurrentMethod().Name, ex);
        ...
        public ResponseOut AddEditChasisSerialMapping(ChasisSerialMapping chasisSerialMapping)
        {
            ...
            try
            {
                ...
                else if (entities.ChasisSerialMappings.Any(x => x.ControllerNo == chasisSerialMapping.ControllerNo && x.MappingId != chasisSerialMapping.MappingId))
                {
                    responseOut.status = ActionStatus.Fail;
        ...
        public List<EmailTemplateType> GetEmailTemplateType()
        {
            ...
            try
            {
                var emailTemplateTypes = entities.EmailTemplateTypes.Where(x => x.Status == true).Select(s => new
                {
        ...
        public ResponseOut ApproveRejectStoreRequisition(StoreRequisition storeRequisition)
        {
            ...
            try
            {
                ...
                {
                    if (entities.StoreRequisitions.Any(x => x.ApprovalStatus == "Approved" && x.ApprovalStatus != null && x.RequisitionId == storeRequisition.RequisitionId))
                    {
                        responseOut.status = ActionStatus.Fail;
                        responseOut.message = ActionMessage.StoreRequisitionAlreadyApprove;
                    }
                    else
                    {
                ...
                {
                    if (entities.StoreRequisitions.Any(x => x.ApprovalStatus == "Rejected" && x.ApprovalStatus != null && x.RequisitionId == storeRequisition.RequisitionId))
                    {
                        responseOut.status = ActionStatus.Fail;
                        responseOut.message = ActionMessage.StoreRequisitionAlreadyRejected;
                    }
                    else
                    {
                        entities.StoreRequisitions.Where(a => a.RequisitionId == storeRequisition.RequisitionId).ToList().ForEach(a =>
        ...
        public ResponseOut ApproveRejectPO(PO po)
        {
            ...
            try
            {
                ...
                else if (po.ApprovalStatus == "Approved")
                {
                    ...
                    //Update vendor id and purchase price in Product Master 
                    entities.proc_UpdatePurchasePriceAndVendorId(po.POId);
        ...
        public ResponseOut AddEditSiteProductOpening(SiteProductOpeningStock siteProductOpeningStock)
        {
            ResponseOut responseOut = new ResponseOut();
            try
            {
                if (entities.SiteProductOpeningStocks.Any(x => x.ProductId == siteProductOpeningStock.ProductId && x.CompanyId == siteProductOpeningStock.CompanyId && x.CompanyBranchId == siteProductOpeningStock.CompanyBranchId && x.CustomerId == siteProductOpeningStock.CustomerId && x.CustomerBranchId == siteProductOpeningStock.CustomerBranchId && x.FinYearId == siteProductOpeningStock.FinYearId && x.SiteOpeningTrnId != siteProductOpeningStock.SiteOpeningTrnId))
                {
                    responseOut.status = ActionStatus.Fail;
                    responseOut.message = ActionMessage.CustomerSiteDuplicateProductOpening;
                }

                else
                {
                    if (siteProductOpeningStock.SiteOpeningTrnId == 0)
                    {
                        siteProductOpeningStock.CreatedDate = DateTime.Now;
                        entities.SiteProductOpeningStocks.Add(siteProductOpeningStock);
                        responseOut.message = ActionMessage.CustomerSiteProductOpeningCreatedSuccess;
                    }
                    else
                    {
                        siteProductOpeningStock.ModifiedBy = siteProductOpeningStock.CreatedBy;
                        siteProductOpeningStock.ModifiedDate = DateTime.Now;

                        entities.SiteProductOpeningStocks.Where(a => a.SiteOpeningTrnId == siteProductOpeningStock.SiteOpeningTrnId).ToList().ForEach(a =>
                        {
                            a.ProductId = siteProductOpeningStock.ProductId;
                            a.FinYearId = siteProductOpeningStock.FinYearId;
                            a.CompanyBranchId = siteProductOpeningStock.CompanyBranchId;
                            a.CustomerId = siteProductOpeningStock.CustomerId;
                            a.CustomerBranchId = siteProductOpeningStock.CustomerBranchId;
                            a.OpeningQty = siteProductOpeningStock.OpeningQty;
                            a.ModifiedBy = siteProductOpeningStock.ModifiedBy;
                            a.ModifiedDate = siteProductOpeningStock.ModifiedDate;

                        });
                        responseOut.message = ActionMessage.CustomerSiteProductOpeningUpdatedSuccess;
                    }
                    entities.SaveChanges();
                    responseOut.status = ActionStatus.Success;
        ...
        #region Site Product Serial
        public ResponseOut AddEditSiteProductSerial(SiteProductSerialDetail siteProductSerialDetail)
        {
            ResponseOut responseOut = new ResponseOut();
            try
            {
                if (entities.SiteProductSerialDetails.Any(x => x.Serial1 == siteProductSerialDetail.Serial1 && x.ProductId == siteProductSerialDetail.ProductId && x.SiteProductSerialId != siteProductSerialDetail.SiteProductSerialId))
                {
                    responseOut.status = ActionStatus.Fail;
                    responseOut.message = ActionMessage.DuplicateSiteProductSerialName;
                }
                else
                {
                    if (siteProductSerialDetail.SiteProductSerialId == 0)
                    {
                        entities.SiteProductSerialDetails.Add(siteProductSerialDetail);
                        responseOut.message = ActionMessage.SiteProductSerialCreatedSuccess;
                    }
                    else
                    {
                        entities.SiteProductSerialDetails.Where(a => a.SiteProductSerialId == siteProductSerialDetail.SiteProductSerialId).ToList().ForEach(a =>
                        {
                            a.ProductId = siteProductSerialDetail.ProductId;
                            a.ProductSerialNo = siteProductSerialDetail.ProductSerialNo;
                            a.Serial1 = siteProductSerialDetail.Serial1;
                            a.Serial2 = siteProductSerialDetail.Serial2;
                            a.Serial3 = siteProductSerialDetail.Serial3;
                            a.ProductSerialStatus = siteProductSerialDetail.ProductSerialStatus;
                        });
                        responseOut.message = ActionMessage.SiteProductSerialUpdatedSuccess;
                    }
                    entities.SaveChanges();
                    responseOut.status = ActionStatus.Success;
        ...
        #region Escalation Master
        public List<EscalationMaster> GetEscalationMasterList()
        {
            List<EscalationMaster> escalationMasterList = new List<EscalationMaster>();
            try
            {
                var escalationMasters = entities.EscalationMasters.Select(s => new
                {
                    EscalationId = s.EscalationId,
                    ProcessType = s.ProcessType,
                    EscalationDays = s.EscalationDays
                }).ToList();
                if (escalationMasters != null && escalationMasters.Count > 0)
                {
                    foreach (var item in escalationMasters)
                    {
                        escalationMasterList.Add(new EscalationMaster { EscalationId = item.EscalationId, ProcessType = item.ProcessType, EscalationDays = item.EscalationDays });
                    }
                }
            }
            catch (Exception ex)
            ...
            return escalationMasterList;
        }

        public ResponseOut AddEditEscalationMaster(EscalationMaster escalationMaster)
        {
            ResponseOut responseOut = new ResponseOut();
            try
            {
                if (entities.EscalationMasters.Any(x => x.EscalationId != escalationMaster.EscalationId && x.ProcessType == escalationMaster.ProcessType))
                {
                    responseOut.status = ActionStatus.Fail;
                    responseOut.message = ActionMessage.DuplicateEscalationMasterProcessType;
                }
                else
                {
                    if (escalationMaster.EscalationId == 0)
                    {
                        entities.EscalationMasters.Add(escalationMaster);
                        responseOut.message = ActionMessage.EscalationMasterCreatedSuccess;
                    }
                    else
                    {
                        entities.EscalationMasters.Where(a => a.EscalationId == escalationMaster.EscalationId).ToList().ForEach(a =>
                        {
                            a.EscalationId = escalationMaster.EscalationId;
                            a.ProcessType = escalationMaster.ProcessType;
                            a.EscalationDays = escalationMaster.EscalationDays;

                        });
                        responseOut.message = ActionMessage.EscalationMasterUpdatedSuccess;
                    }
                    entities.SaveChanges();
                    responseOut.status = ActionStatus.Success;
        ...
        {
            List<TimeType> timeTypeList = new List<TimeType>();
            try
            {
                var timeTypes = entities.TimeTypes.Select(s => new
                {
                    TimeTypeId = s.TimeTypeId,
                    TimeTypeName = s.TimeTypeName
                }).ToList();
                if (timeTypes != null && timeTypes.Count > 0)
                {
                    foreach (var item in timeTypes)
                    {
                        timeTypeList.Add(new TimeType { TimeTypeId = item.TimeTypeId, TimeTypeName = item.TimeTypeName });
                    }
                }
            }
            catch (Exception ex)
        ...
        public decimal GetFoodAndTeaExpensesByTimeType(string timeTypeName)
        {
            ...
            try
            {
                FoodAndTeaExpense = Convert.ToDecimal(entities.TimeTypes.Where(s => s.TimeTypeName.Trim().ToUpper() == timeTypeName.Trim().ToUpper()).Select(x => x.TimeTypeRate).FirstOrDefault());
            }
            catch (Exception ex)
            {
                Logger.SaveErrorLog(this.ToString(), MethodBase.GetCurrentMethod().Name, ex);
        ...
        public decimal GetVagesByEmployeeId(int employeeId)
        {
            ...
            try
            {
                Vages = Convert.ToDecimal(entities.EmployeePayInfoes.Where(e => e.EmployeeId == employeeId).Select(v => v.BasicPay).FirstOrDefault());
            }
            catch (Exception ex)
            {
                Logger.SaveErrorLog(this.ToString(), MethodBase.GetCurrentMethod().Name, ex);
        ...
        #region Unit
        public ResponseOut AddEditUnit(Unit unitMod)
        {
            ResponseOut responseOut = new ResponseOut();
            try
            {
                if (unitMod.UnitId == 0)
                {
                    entities.Units.Add(unitMod);
                    responseOut.message = ActionMessage.UnitCreatedSuccess;
                }
                else
                {
                    entities.Units.Where(a => a.UnitId == unitMod.UnitId).ToList().ForEach(a =>
                    {

                        a.UnitName = unitMod.UnitName;
                        a.UnitShortName = unitMod.UnitShortName;
                        a.AllowDecimal = unitMod.AllowDecimal;
                        a.IsMultipleUnit = unitMod.IsMultipleUnit;
                        a.UnitValue = unitMod.UnitValue;
                        a.ParentId = unitMod.ParentId;
                    });
                    responseOut.message = ActionMessage.UnitUpdatedSuccess;
                }
                entities.SaveChanges();
                responseOut.trnId = unitMod.UnitId;
                responseOut.status = ActionStatus.Success;
            }
            catch (Exception ex)
            {
        ...
        public List<Unit> GetBaseUnitList()
        {
            List<Unit> cityList = new List<Unit>();
            try
            {
                cityList = entities.Units.Where(x => !x.IsMultipleUnit).ToList();
            }
            catch (Exception ex)
            {
                Logger.SaveErrorLog(this.ToString(), MethodBase.GetCurrentMethod().Name, ex);
