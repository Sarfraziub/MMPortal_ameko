...
namespace Portal.Controllers
{
    ...
    public class ActivityCalenderController : BaseController
    {
        ...
        [HttpGet]
        public PartialViewResult GetActivityCalenderList( int calenderId = 0, string fromDate = "", string toDate = "", string Status = "")
        {
            List<ActivityCalenderViewModel> activityCalender = new List<ActivityCalenderViewModel>();
            ActivityCalenderBL activitycalenderBL = new ActivityCalenderBL();
            try
            {
                activityCalender = activitycalenderBL.GetActivityCalenderList(calenderId, fromDate, toDate, Status);
            }
            catch (Exception ex)
            {
                Logger.SaveErrorLog(this.ToString(), MethodBase.GetCurrentMethod().Name, ex);
