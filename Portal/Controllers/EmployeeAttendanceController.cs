...
namespace Portal.Controllers
{
    ...
    public class EmployeeAttendanceController : BaseController
    {
        ...
        public ActionResult AddEditEmployeeAttendance(int employeeId, string attendanceDate, string presentAbsent, string inTime, string outTime, string attendanceStatus)
        {
            ...
            try
            {
                if (attendanceDate !="")
                {
                    responseOut = employeeAttendanceBL.AddEditEmployeeAttendance(ContextUser.CompanyId, employeeId, attendanceDate, presentAbsent, inTime, outTime, attendanceStatus, ContextUser.UserId);
                }
                else
                {
        ...
        [HttpGet]
        public PartialViewResult GetEmployeeAttendanceList(int employeeId = 0, string attendanceDate = "", int departmentId = 0, int designationId = 0)
        {
            List<HR_EmployeeAttendanceViewModel> employeeAttendance = new List<HR_EmployeeAttendanceViewModel>();
            EmployeeAttendanceBL employeeAttendanceBL = new EmployeeAttendanceBL();
            try
            {
                employeeAttendance = employeeAttendanceBL.GetEmployeeMarkAttendanceList(employeeId, attendanceDate, departmentId, designationId);
            }
            catch (Exception ex)
            {
                Logger.SaveErrorLog(this.ToString(), MethodBase.GetCurrentMethod().Name, ex);
        ...
        public ActionResult EmployeeAttendanceReport(int month = 0, int year = 0, string reportType = "PDF")
        {
            ...
            dt = employeeAttendanceBL.GetEmployeeAttendanceReport(month, year, ContextUser.RoleId);
