...
namespace Portal.Controllers
{
    ...
    public class EmployeeClaimApplicationController : BaseController
    {
        ...
        [HttpGet]
        public PartialViewResult GetEmployeeClaimApplicationList(string applicationNo = "", int employeeId = 0, int claimTypeId =0, string claimStatus = "0", string fromDate = "", string toDate = "", int essEmployeeId = 0, string essEmployeeName = "")
        {
            ...
            try
            {
                ViewData["essEmployeeId"] = essEmployeeId;
                ViewData["essEmployeeName"] = essEmployeeName;
                employeeClaimApplicationViewModel = employeeClaimApplicationBL.GetEmployeeClaimApplicationList(applicationNo, essEmployeeId, claimTypeId, claimStatus, fromDate, toDate,ContextUser.CompanyId);
            }
            catch (Exception ex)
            {
                Logger.SaveErrorLog(this.ToString(), MethodBase.GetCurrentMethod().Name, ex);
        ...
        [HttpGet]
        public PartialViewResult GetEmployeeClaimApplicationApprovalList(string applicationNo = "", int employeeId = 0, int claimTypeId =0, string claimStatus = "0", string fromDate = "", string toDate = "")
        {
            List<EmployeeClaimApplicationViewModel> employeeClaimApplicationViewModel = new List<EmployeeClaimApplicationViewModel>();           
            EmployeeClaimApplicationBL employeeClaimApplicationBL = new EmployeeClaimApplicationBL();
            try
            {
                employeeClaimApplicationViewModel = employeeClaimApplicationBL.GetEmployeeClaimApplicationApprovalList(applicationNo, employeeId, claimTypeId, claimStatus, fromDate, toDate, ContextUser.CompanyId);
            }
            catch (Exception ex)
            {
                Logger.SaveErrorLog(this.ToString(), MethodBase.GetCurrentMethod().Name, ex);
