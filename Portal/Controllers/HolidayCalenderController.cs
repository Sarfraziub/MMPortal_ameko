...
namespace Portal.Controllers
{
    ...
    public class HolidayCalenderController : BaseController
    {
        ...
        [HttpGet]
        public PartialViewResult GetHolidayCalenderList(int calenderId = 0, int holidaytypeId = 0, string fromDate = "", string toDate = "", string Status = "")
        {
            List<HolidayCalenderViewModel> holidayCalender = new List<HolidayCalenderViewModel>();
            HolidayCalenderBL holidaycalenderBL = new HolidayCalenderBL();
            try
            {
                holidayCalender = holidaycalenderBL.GetHolidayCalenderList(calenderId, holidaytypeId,fromDate, toDate, Status);
            }
            catch (Exception ex)
            {
                Logger.SaveErrorLog(this.ToString(), MethodBase.GetCurrentMethod().Name, ex);
