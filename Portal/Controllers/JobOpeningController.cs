...
namespace Portal.Controllers
{
    ...
    public class JobOpeningController : BaseController
    {
        ...
        [HttpGet]
        public PartialViewResult GetResourceRequisitionList(string requisitionNo = "", int positionLevelId = 0, string priorityLevel = "", int positionTypeId = 0, int departmentId = 0, string approvalStatus = "Approved", string fromDate = "",string toDate="")
        {
            ...
            try
            {
                resourceRequisitions = rrBL.GetResourceRequisitionList(requisitionNo, positionLevelId, priorityLevel, positionTypeId, departmentId, approvalStatus, fromDate, toDate,ContextUser.CompanyId);
            }
            catch (Exception ex)
            {
                Logger.SaveErrorLog(this.ToString(), MethodBase.GetCurrentMethod().Name, ex);
