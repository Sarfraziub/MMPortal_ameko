...
namespace Portal.Controllers
{
    ...
    public class PMSGoalController : BaseController
    {
        ...
        [HttpGet]
        public PartialViewResult GetGoalList(string goalName = "", int sectionId = 0, int goalCategoryId = 0, int performanceCycleId = 0, string goalStatus = "", string fromDate = "", string toDate = "")
        {
            List<PMSGoalViewModel> pMSGoalViewModel = new List<PMSGoalViewModel>();
            GoalBL goalBL = new GoalBL();           
            try
            {
                pMSGoalViewModel = goalBL.GetGoalList(goalName, sectionId, goalCategoryId, performanceCycleId, goalStatus, fromDate, toDate, ContextUser.CompanyId);
            }
            catch (Exception ex)
            {
                Logger.SaveErrorLog(this.ToString(), MethodBase.GetCurrentMethod().Name, ex);
