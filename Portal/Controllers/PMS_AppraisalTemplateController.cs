...
namespace Portal.Controllers
{
    ...
    public class PMS_AppraisalTemplateController : BaseController
    {
        ...
        [HttpGet]
        public PartialViewResult GetAppraisalTemplateLists(string templateName = "", int department = 0, int designation = 0,  string appraisaltemplateStatus = "")
        {
            List<PMS_AppraisalTemplateViewModel> appraisalTemplates = new List<PMS_AppraisalTemplateViewModel>();
            PMS_AppraisalTemplateBL appraisalTemplateBL = new PMS_AppraisalTemplateBL();
            try
            {
                appraisalTemplates = appraisalTemplateBL.GetAppraisalTemplateLists(templateName, department, designation, ContextUser.CompanyId, appraisaltemplateStatus);
            }
            catch (Exception ex)
            {
                Logger.SaveErrorLog(this.ToString(), MethodBase.GetCurrentMethod().Name, ex);
