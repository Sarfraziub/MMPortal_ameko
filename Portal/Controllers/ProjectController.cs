...
namespace Portal.Controllers
{
    public class ProjectController : BaseController
    {
        ...
        [HttpGet] 
        public PartialViewResult GetProjectList(string projectName = "", int projectCode = 0, int customerId = 0 ,int customerBranchId=0,string projectStatus="")
        {
            List<ProjectViewModel> projects = new List<ProjectViewModel>();
            ProjectBL projectBL = new ProjectBL();
            try
            {
                projects = projectBL.GetProjectList(projectName, projectCode, customerId, customerBranchId, projectStatus);
            }
            catch (Exception ex)
            {
                Logger.SaveErrorLog(this.ToString(), MethodBase.GetCurrentMethod().Name, ex);
