...
namespace Portal.Controllers
{
    public class PurchaseQuotationController : BaseController
    {
        ...
        [ValidateRequest(true, UserInterfaceHelper.Add_Edit_PurchaseQuotation, (int)AccessMode.ViewAccess, (int)RequestMode.GetPost)]
        public ActionResult ListPurchaseQuotation()
        {
            try
            {
                FinYearViewModel finYear = Session[SessionKey.CurrentFinYear] != null ? (FinYearViewModel)Session[SessionKey.CurrentFinYear] : new FinYearViewModel();
        ...
        [HttpGet]
        public PartialViewResult GetPurchaseQuotationList(string quotationNo = "", string vendorName = "", string refNo = "", string fromDate = "", string toDate = "", string displayType = "", string approvalStatus = "")
        {
            List<PurchaseQuotationViewModel> quotations = new List<PurchaseQuotationViewModel>();
            PurchaseQuotationBL purchaseQuotationBL = new PurchaseQuotationBL();
            try
            {
                quotations = purchaseQuotationBL.GetPurchaseQuotationList(quotationNo, vendorName, refNo, fromDate, toDate, ContextUser.CompanyId, displayType, approvalStatus);
            }
            catch (Exception ex)
            {
                Logger.SaveErrorLog(this.ToString(), MethodBase.GetCurrentMethod().Name, ex);
        ...
        public PartialViewResult GetPurchaseIndentList(string indentNo = "", string indentType = "", string customerName = "", int companyBranchId = 0, string fromDate = "", string toDate = "", string displayType = "", string approvalStatus = "0")
        {
            ...
            try
            {
                indents = purchaseQuotationBL.GetPurchaseQuotationIndentList(indentNo, indentType, customerName, companyBranchId, Convert.ToDateTime(fromDate), Convert.ToDateTime(toDate), ContextUser.CompanyId, displayType, approvalStatus);
            }
            catch (Exception ex)
                {
                Logger.SaveErrorLog(this.ToString(), MethodBase.GetCurrentMethod().Name, ex);
        ...
        public PartialViewResult GetPurchaseIndentProductList(List<PurchaseIndentProductDetailViewModel> purchaseIndentProducts, long indentId)
        {
            ...
            try
            {
                ...
                {
                    purchaseIndentProducts = purchaseQuotationBL.GetPurchaseQuotationIndentProductList(indentId);
                }
        ...
        [ValidateRequest(true, UserInterfaceHelper.Add_Edit_QuotationComparison, (int)AccessMode.ViewAccess, (int)RequestMode.GetPost)]
