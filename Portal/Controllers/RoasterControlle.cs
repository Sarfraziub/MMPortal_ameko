...
namespace Portal.Controllers
{
    ...
    public class RoasterController : BaseController
    {
        ...
        [HttpGet]
        public PartialViewResult GetRoasterList(string roasterName = "", string fromDate = "", string toDate = "", string roasterType="", int department=0,  string roasterStatus = "")
        {
            List<HR_RoasterViewModel> roasters = new List<HR_RoasterViewModel>();
            RoasterBL roasterBL = new RoasterBL();
            try
            {
                roasters = roasterBL.GetRoasterList(roasterName, fromDate, toDate, roasterType, department, ContextUser.CompanyId, roasterStatus);
            }
            catch (Exception ex)
            {
                Logger.SaveErrorLog(this.ToString(), MethodBase.GetCurrentMethod().Name, ex);
        ...
        [ValidateRequest(true, UserInterfaceHelper.UpdateDateWiseShift, (int)AccessMode.ViewAccess, (int)RequestMode.GetPost)]
        ...
        public ActionResult UpdateDateWiseShift(int rosterId, int shiftId, string startDate,string endDate, List<HR_EmployeeRosterViewModel> employeeRosters)
        {
            ResponseOut responseOut = new ResponseOut();
            RoasterBL roasterBL = new RoasterBL();
            try
            {
                if (rosterId != 0)
                {
                    responseOut = roasterBL.UpdateEmployeeRosterShift(rosterId,shiftId,startDate,endDate,ContextUser.UserId,employeeRosters);
                }
                else
                {
                    responseOut.message = ActionMessage.ProbleminData;
