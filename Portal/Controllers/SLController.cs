...
namespace Portal.Controllers
{
    ...
    public class SLController :BaseController
    {
        ...
        [HttpGet]
        public PartialViewResult GetSLList(string SLHead = "",  string SLCode = "", int SLTypeId = 0, int CostCenterId = 0, string Status = "")
        {
            List<SLViewModel> sL = new List<SLViewModel>();
            SLBL sLBL = new SLBL();
            try
            {
                sL = sLBL.GetSLList(SLHead,SLCode, SLTypeId,CostCenterId,ContextUser.CompanyId,Status);
            }
            catch (Exception ex)
            {
                Logger.SaveErrorLog(this.ToString(), MethodBase.GetCurrentMethod().Name, ex);
