...
namespace Portal.Controllers
{
    ...
    public class ShiftController : BaseController
    {
        ...
        [HttpGet]
        public PartialViewResult GetShiftList(string shiftName="",int shiftTypeId = 0, string shiftTypeStatus = "")
        {
            List<ShiftViewModel> shiftViewModel = new List<ShiftViewModel>();
            HRShiftBL hRShiftBL = new HRShiftBL(); 
            try
            {
                shiftViewModel = hRShiftBL.GetShiftList(shiftName, shiftTypeId, shiftTypeStatus, ContextUser.CompanyId);
            }
            catch (Exception ex)
            {
                Logger.SaveErrorLog(this.ToString(), MethodBase.GetCurrentMethod().Name, ex);
