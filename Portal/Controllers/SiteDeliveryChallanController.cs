...
namespace Portal.Controllers
{
    ...
    public class SiteDeliveryChallanController : BaseController
    {
        #region SiteDeliveryChallan

        //
        // GET: /SiteDeliveryChallan/

        [ValidateRequest(true, UserInterfaceHelper.Add_Edit_SiteDeliveryChallan, (int)AccessMode.ViewAccess, (int)RequestMode.GetPost)]
        public ActionResult AddEditSiteDeliveryChallan(int siteChallanId = 0, int accessMode = 3)
        {
            try
            {
                ...
                if (siteChallanId != 0)
                {
                    ViewData["siteChallanId"] = siteChallanId;
                    ViewData["accessMode"] = accessMode;
                    
                }
                else
                {
                    ViewData["siteChallanId"] = 0;
        ...
        [HttpPost]
        [ValidateRequest(true, UserInterfaceHelper.Add_Edit_SiteDeliveryChallan, (int)AccessMode.AddAccess, (int)RequestMode.Ajax)]
        public ActionResult AddEditSiteDeliveryChallan(SiteDeliveryChallanViewModel siteChallanViewModel, List<SiteChallanProductViewModel> siteChallanProducts,  List<SiteChallanTermsViewModel> siteChallanTerms, List<SiteDeliveryChallanSupportingDocumentViewModel> siteDeliveryChallanDocuments)
        {
            ResponseOut responseOut = new ResponseOut();
            SiteDeliveryChallanBL siteDeliverychallanBL = new SiteDeliveryChallanBL();
            try
            {
                if (siteChallanViewModel != null)
                {
                    siteChallanViewModel.CreatedBy = ContextUser.UserId;
                    siteChallanViewModel.CompanyId = ContextUser.CompanyId;
                    siteChallanViewModel.FinYearId = Session[SessionKey.CurrentFinYear] != null ? ((FinYearViewModel)Session[SessionKey.CurrentFinYear]).FinYearId : DateTime.Now.Year;
                    responseOut = siteDeliverychallanBL.AddEditSiteDeliveryChallan(siteChallanViewModel, siteChallanProducts, siteChallanTerms, siteDeliveryChallanDocuments);
        ...
        [ValidateRequest(true, UserInterfaceHelper.Add_Edit_SiteDeliveryChallan, (int)AccessMode.ViewAccess, (int)RequestMode.GetPost)]
        public ActionResult ListSiteChallan()
        {
            try
            {
                FinYearViewModel finYear = Session[SessionKey.CurrentFinYear] != null ? (FinYearViewModel)Session[SessionKey.CurrentFinYear] : new FinYearViewModel();
        ...
        [HttpGet]
        public PartialViewResult GetSiteChallanList(string siteChallanNo = "", string customerName = "", string dispatchrefNo = "", string fromDate = "", string toDate = "",string approvalStatus="")
        {
            List<SiteDeliveryChallanViewModel> siteDeliverychallans = new List<SiteDeliveryChallanViewModel>();
            SiteDeliveryChallanBL siteDeliverychallanBL = new SiteDeliveryChallanBL();
            try
            {
                siteDeliverychallans = siteDeliverychallanBL.GetSiteChallanList(siteChallanNo, customerName, dispatchrefNo, fromDate, toDate, approvalStatus, ContextUser.CompanyId);
            }
            catch (Exception ex)
            {
                Logger.SaveErrorLog(this.ToString(), MethodBase.GetCurrentMethod().Name, ex);
            }
            return PartialView(siteDeliverychallans);
        }

        [HttpGet]
        public JsonResult GetSiteChallanDetail(long siteChallanId)
        {
            SiteDeliveryChallanBL siteDeliverychallanBL = new SiteDeliveryChallanBL();
            SiteDeliveryChallanViewModel siteDeliverychallan = new SiteDeliveryChallanViewModel();
            try
            {
                siteDeliverychallan = siteDeliverychallanBL.GetSiteChallanDetail(siteChallanId);
            }
            catch (Exception ex)
            {
                Logger.SaveErrorLog(this.ToString(), MethodBase.GetCurrentMethod().Name, ex);
            }
            return Json(siteDeliverychallan, JsonRequestBehavior.AllowGet);
        }
        [HttpPost]
        public PartialViewResult GetSiteChallanProductList(List<SiteChallanProductViewModel> siteChallanProducts, long siteChallanId)
        {
            SiteDeliveryChallanBL siteDeliverychallanBL = new SiteDeliveryChallanBL();
            try
            {
                if (siteChallanProducts == null)
                {
                    siteChallanProducts = siteDeliverychallanBL.GetSiteChallanProductList(siteChallanId);
                }
            ...
            return PartialView(siteChallanProducts);
        }
        //[HttpPost]
        ...
        //}
        [HttpPost]
        public PartialViewResult GetSiteChallanTermList(List<SiteChallanTermsViewModel> siteChallanTerms, long siteChallanId)
        {
            SiteDeliveryChallanBL siteDeliverychallanBL = new SiteDeliveryChallanBL();
            try
            {
                if (siteChallanTerms == null)
                {
                    siteChallanTerms = siteDeliverychallanBL.GetSiteChallanTermList(siteChallanId);
                }
            ...
            return PartialView(siteChallanTerms);
        }
        ...
        public PartialViewResult GetSINList(string sinNo = "", string requisitionNo = "", string jobno = "", int companyBranchId = 0, string fromDate = "", string toDate = "", string sINStatus = "")
        {
            ...
            try
            {
                stns = siteDeliverychallanBL.GetDeliveryChallanSINList(sinNo, requisitionNo, jobno, companyBranchId, fromDate, toDate, ContextUser.CompanyId, sINStatus);
            }
            catch (Exception ex)
            {
                Logger.SaveErrorLog(this.ToString(), MethodBase.GetCurrentMethod().Name, ex);
        ...
        public ActionResult Report(long siteChallanId, string reportType = "PDF")
        {
            LocalReport lr = new LocalReport();
            SiteDeliveryChallanBL sitechallanBL = new SiteDeliveryChallanBL();
            string path = Path.Combine(Server.MapPath("~/RDLC"), "SiteChallanPrint.rdlc");
            ...
            dt = sitechallanBL.GetSiteChallanDetailTable(siteChallanId);
            ...
            ReportDataSource rdProduct = new ReportDataSource("DataSetProduct", sitechallanBL.GetSiteChallanProductListDataTable(siteChallanId));
            //ReportDataSource rdTax = new ReportDataSource("DataSetTax", soBL.GetSOTaxList(soId));
            ReportDataSource rdTerms = new ReportDataSource("DataSetTerms", sitechallanBL.GetSiteChallanTermListDataTable(siteChallanId));
        ...
        [HttpPost]
        public PartialViewResult GetSiteDeliveryChallanSupportingDocumentList(List<SiteDeliveryChallanSupportingDocumentViewModel> siteDeliveryChallanDocuments, long siteChallanId)
        { 
            SiteDeliveryChallanBL siteDeliveryChallanBL = new SiteDeliveryChallanBL();
            try
            {
                if (siteDeliveryChallanDocuments == null)
                {
                    siteDeliveryChallanDocuments = siteDeliveryChallanBL.GetSiteDeliveryChallanSupportingDocumentList(siteChallanId);
                }
            ...
            return PartialView(siteDeliveryChallanDocuments);
        }
