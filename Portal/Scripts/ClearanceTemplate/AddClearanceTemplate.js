...
function SaveData() {
    ...
    $.ajax({
        url: "../ClearanceTemplate/AddEditClearanceTemplate",
        cache: false,
        type: "POST",
        dataType: "json",
        data: JSON.stringify(requestData),
        ...
        success: function (data) {
            if (data.status == "SUCCESS") {
                ...
                setTimeout(
                   function () {
                       window.location.href = "../ClearanceTemplate/AddEditClearanceTemplate?ClearanceTemplateId=" + data.trnId + "&AccessMode=2";
                   }, 2000);
