...
.autocomplete("instance")._renderItem = function (ul, item) {
    ...
    $("#txtRefDate,#txtSearchFromDate,#txtSearchToDate").datepicker({
        changeMonth: true,
        changeYear: true,
        dateFormat: 'dd-M-yy',
        yearRange: '-10:+100',
...
function SaveData() {
    ...
    var requestData = { piViewModel: piViewModel, piProducts: piProductList, piTaxes: piTaxList, piTerms: piTermList, piDocuments: piDocumentList };
    $.ajax({
...
function SearchPO() {
    ...
    var requestData = { poNo: txtSearchPONo.val().trim(), vendorName: txtVendorName.val().trim(), refNo: txtRefNo.val().trim(), fromDate: txtFromDate.val(), toDate: txtToDate.val(), approvalStatus: "0", displayType: "Popup" };
    $.ajax({
...
function ResetPage() {
    if (confirm("Are you sure to reset all fields?")) {
        window.location.href = "../PI/AddEditPI?accessMode=1";
    }
}