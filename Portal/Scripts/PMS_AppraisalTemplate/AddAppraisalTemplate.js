...
function SaveData() {
    ...
    $.ajax({
        url: "../PMS_AppraisalTemplate/AddEditAppraisalTemplate",
        cache: false,
        type: "POST",
        dataType: "json",
        data: JSON.stringify(requestData),
        ...
        success: function (data) {
            if (data.status == "SUCCESS") {
                ...
                setTimeout(
                   function () {
                       window.location.href = "../PMS_AppraisalTemplate/AddEditAppraisalTemplate?TemplateId=" + data.trnId + "&AccessMode=2";
                   }, 2000);
