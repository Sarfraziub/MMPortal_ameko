...
function GetProductDetail(productId) {
    $.ajax({
        ...
        success: function (data) {
            ...
            $('#ddlBaseUnit').val(data.UnitId);
            $('#baseUnitLbl').text($('#ddlBaseUnit').find('option:selected').text());
        },
        error: function (Result) {
...
function SaveData() {
    ...
    //if (ddlBaseUnit.val() == "" || ddlBaseUnit.val() == "0") {
    //    ShowModel("Alert", "Please select Base Unit")
    //    ddlBaseUnit.focus();
    //    return false;
    //}
    ...
    var GSTExempt = false;
    if (chkGSTExempt.prop("checked") == true) { GSTExempt = true; }
    else { GSTExempt = false; }
...
function BindBaseUnitList() {
    $.ajax({
        ...
        asnc: false,
        success: function (data) {
            $('#ddlBaseUnit').find('option').remove().end();
            $('#ddlBaseUnit1').find('option').remove().end();
            $("#ddlBaseUnit").append($("<option></option>").val(0).html("-Select Base Unit-"));
            $("#ddlBaseUnit1").append($("<option></option>").val(0).html("-Select Base Unit-"));
            $.each(data, function (i, item) {

                $("#ddlBaseUnit").append($("<option></option>").val(item.UnitId).html(item.UnitName + " (" + item.UnitShortName + ")"));
                $("#ddlBaseUnit1").append($("<option></option>").val(item.UnitId).html(item.UnitName + " (" + item.UnitShortName + ")"));
            });
        },
        error: function (Result) {
            $("#ddlBaseUnit").append($("<option></option>").val(0).html("-Select Base Unit-"));
...
function ShowOtherUnits() {
    ...
    if (chkIsAddMultipleLimit.prop("checked") == true) {
        ...
        //$("#divMultipleUnits").css("visibility", "hidden");
...
function UnitNameChange() {
    ...
    $("#lblUnitShortName").html(unitName);
}
