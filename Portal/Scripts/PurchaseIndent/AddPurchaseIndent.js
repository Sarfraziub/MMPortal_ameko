...
function BindCompanyBranchList() {
    ...
    $.ajax({
        ...
        error: function (Result) {
            $("#ddlCompanyBranch").append($("<option></option>").val(0).html("-Select Branch-"));
        }
    });
}
...
function AddProduct(action) {
    ...
    if (parseFloat(txtQuantity.val().trim()) > parseFloat(txtAvailableStock.val().trim())) {
        ShowModel("Alert", "Quantity cannot be greater than Available Stock Quantity")
        txtQuantity.focus();
        return false;
    }
...
function SaveData() {
    ...
    if (txtIndentByUser.val() == "" || hdnUserId.val() == "0")
    {
        ShowModel("Alert", "Enter Indent Raised By ");
...
function GetPurchaseIndentDetail(indentId) {
    $.ajax({
        ...
        success: function (data) {
            ...
            if (data.RequisitionNo != null && data.RequisitionNo != "") {
...
function SearchRequisition() {
    ...
    $.ajax({
        url: "../PurchaseIndent/GetStoreRequisitionList",
        data: requestData,
        dataType: "html",
        type: "GET",
        error: function (err) {
...
function SelectRequisition(requisitionId, requisitionNo, requisitionDate, customerName, customerId, customerCode, customerBranchName, customerBranchId) {
    ...
    BindCustomerBranchList(customerId, customerBranchId);
   // $("#btnAddNewProduct").hide();
    //$("#hdnCustomerId").val(customerId);
...
function GetRequisitionProductList(sinProducts) {
    ...
    $.ajax({
        url: "../PurchaseIndent/GetStoreRequisitionProductList",
        cache: false,
        data: JSON.stringify(requestData),
        dataType: "html",
        contentType: "application/json; charset=utf-8",
...
function ResetPage() {
    if (confirm("Are you sure to reset all fields?")) {
        window.location.href = "../PurchaseIndent/AddEditPurchaseIndent?accessMode=1";
    }
}