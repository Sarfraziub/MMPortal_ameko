    ...
    $.ajax({
        ...
        success: function (data) {
            if (data.status == "SUCCESS") {
                ...
                setTimeout(
                   function () {
                       window.location.href = "../PayrollOtherEarningDeduction/AddEditPayrollOtherEarningDeduction?monthlyInputId=" + data.trnId + "&AccessMode=3";
                   }, 2000);
...
function ResetPage() {
    if (confirm("Are you sure to reset all fields?")) {
        window.location.href = "../PayrollOtherEarningDeduction/AddEditPayrollOtherEarningDeduction?accessMode=1";
    }
}