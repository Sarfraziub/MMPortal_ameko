...
function SaveData() {
    ...
    var ddlIndentApprovel = $("#ddlIndentApprovel");
            ...
            if (ddlIndentApprovel.val() == "0" || ddlIndentApprovel.val() == undefined) {
                ShowModel("Alert", "Please Enter Purchase Indent Approvel Status")
                return false;
            ...
            if (hdnIndentId.val() != null && hdnIndentId.val() != 0) {
                accessMode = 2;//Edit Mode
            }

            var requestData = { purchaseIndentViewModel: purchaseIndentViewModel };
        $.ajax({
            url: "../PurchaseIndent/ApprovalPurchaseIndent?AccessMode=" + accessMode + "",
            cache: false,
            type: "POST",
            dataType: "json",
            data: JSON.stringify(requestData),
...
function EnableDisableRejectReason() {
    ...
    if (approvalStatus == "Rejected") {
        ...
        $("#spnTxtRejectReason").css("display", 'inline');
    }
    else {
        $("#txtRejectedReason").attr("disabled", true);
        $("#txtRejectedReason").val("");
        $("#spnTxtRejectReason").css("display", 'none');
    }
}
