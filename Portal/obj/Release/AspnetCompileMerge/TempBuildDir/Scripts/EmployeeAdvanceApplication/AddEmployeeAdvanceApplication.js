    ...
    $.ajax({
        ...
        success: function (data) {
            if (data.status == "SUCCESS") {
                ...
                setTimeout(
                   function () {
                       window.location.href = "../EmployeeAdvanceApp/AddEditEmployeeAdvanceApp?applicationId=" + data.trnId + "&essEmployeeId=" + hdnEssEmployeeId.val() + "&essEmployeeName=" + hdnEssEmployeeName.val() + "&AccessMode=3";
                   }, 2000);
...
function ResetPage() {
    if (confirm("Are you sure to reset all fields?")) {
        window.location.href = "../EmployeeAdvanceApp/AddEditEmployeeAdvanceApp?accessMode=1";
    }
}