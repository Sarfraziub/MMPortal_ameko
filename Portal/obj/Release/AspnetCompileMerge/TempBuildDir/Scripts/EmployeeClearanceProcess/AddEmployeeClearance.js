...
function SaveData() {
    ...
    $.ajax({
        url: "../EmployeeClearanceProcess/AddEditEmployeeClearance",
        cache: false,
        type: "POST",
        dataType: "json",
        data: JSON.stringify(requestData),
        ...
        success: function (data) {
            if (data.status == "SUCCESS") {
                ...
                setTimeout(
                   function () {
                       window.location.href = "../EmployeeClearanceProcess/AddEditEmployeeClearance?employeeClearanceId=" + data.trnId + "&AccessMode=2";
                   }, 2000);
