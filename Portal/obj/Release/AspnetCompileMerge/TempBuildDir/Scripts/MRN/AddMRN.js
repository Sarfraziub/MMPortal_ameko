...
function AddProduct(action) {
    ...
    var pendingQty = parseFloat(txtQuantity.val() - txtAcceptQuantity.val()).toFixed(2);
    ...
    $('#tblProductList tr').each(function (i, row) {
        ...
        var pendingQuantity = $row.find("#hdnPendingQuantity").val();// hdnPendingQantity
        ...
        if (productName != undefined) {
            if (action == 1 || (hdnSequenceNo.val() != sequenceNo)) {
                ...
                //hdnMRNProductDetailId.val() == mrnProductDetailId &&
...
function SaveData() {
    ...
    if ($("#hdnPOId").val() != 0)
    {
        if (Date.parse($("#txtMRNDate").val()) < Date.parse($("#txtPODate").val()))
        {
            ShowModel('Alert', 'MRN date should be greater than or equal to PO Date.');
            $("#txtMRNDate").focus();
            return false;
        }
    }

    if (txtPONo.val().trim() == "" && $("#hdnPOId").val() == 0)
    {
...
function SearchPO() {
    ...
    var requestData = { poNo: txtInvoiceNo.val().trim(), vendorName: txtVendorName.val().trim(), refNo: txtRefNo.val().trim(), fromDate: txtFromDate.val(), toDate: txtToDate.val(), approvalStatus: "0", displayType: "Popup" };
    $.ajax({
        url: "../MRN/GetPOListForMRN",
        data: requestData,
        dataType: "html",
        type: "GET",
        error: function (err) {
...
function SelectPO(invoiceId, invoiceNo, invoiceDate, vendorId, vendorCode, vendorName) {
    //$("#txtInvoiceNo").val(invoiceNo);
    //$("#txtInvoiceDate").val(invoiceDate);
    //$("#hdnVendorId").val(vendorId);
    //$("#txtVendorCode").val(vendorCode);
    ...
    GetPODetail(invoiceId);
    var poProducts = [];
    GetPOProductList(poProducts, invoiceId);
}
function GetPODetail(invoiceId) {
    $.ajax({
        type: "GET",
        asnc: false,
        url: "../PO/GetPODetail",
...
function GetPOProductList(poProducts, invoiceId) {

    var requestData = { poProducts: poProducts, invoiceId: invoiceId };
    $.ajax({
        url: "../MRN/GetPOProductListForMRN",
        cache: false,
        data: JSON.stringify(requestData),
        dataType: "html",
        contentType: "application/json; charset=utf-8",
...
function ResetPage() {
    if (confirm("Are you sure to reset all fields?")) {
        window.location.href = "../MRN/AddEditMRN?accessMode=1";
    }
}