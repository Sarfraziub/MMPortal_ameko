...
.autocomplete("instance")._renderItem = function (ul, item) {
    ...
    $("#txtPODate").datepicker({
        ...
        yearRange: '-10:+100',
        onSelect: function (selected) {
            var poDate = Date.parse($("#txtPODate").val().trim());
            var indentDate = Date.parse($("#txtIndentDate").val().trim()); 
            var deliveryDate = Date.parse($("#txtDeliveryDate").val());
            if (indentDate != null) {
                if (poDate < indentDate) {
                    $("#txtPODate").notify("PO date should be greater than or equal to Indent Date.", {position:'top'});
                    $("#txtPODate").val('');
                    $("#txtPODate").focus();
                }
            }
    ...
    $("#txtDeliveryDate").datepicker({
        ...
        onSelect: function (selected) {
            var poDate = Date.parse($("#txtPODate").val().trim());
            var deliveryDate = Date.parse($("#txtDeliveryDate").val().trim());
            if (poDate != null) {
                if (deliveryDate < poDate) {
                    $("#txtDeliveryDate").notify("Delivery Date should be greater than or equal to PO Date.", { position: 'top' });
                    $("#txtDeliveryDate").val('');
    ...
    if (hdnPOId.val() != "" && hdnPOId.val() != "0" && hdnAccessMode.val() != "" && hdnAccessMode.val() != "0") {
       ...
       function () {
        ...
        if (hdnAccessMode.val() == "3") {
            ...
            $(".editonly").hide();
            if ($(".editonly").hide()) {
                $('#txtQuotationDate').removeClass("col-sm-3 control-label").addClass("col-sm-4 control-label");
                $('#lblIndentDate').removeClass("col-sm-3 control-label").addClass("col-sm-4 control-label");
                $('#lblVendorCode').removeClass("col-sm-3 control-label").addClass("col-sm-4 control-label");
            }
        }
        else {
            $("#btnSave").hide();
...
function BindConsigneeBranchList(consigneeId, CustomerBranchId) {
...
function GetPODetail(poId) {
    $.ajax({
        ...
        success: function (data) {
            ...
            BindConsigneeBranchList(data.ConsigneeId, data.CustomerBranchId);
...
function SaveData() {
    ...
    var CustomerBranchId = $("#ddlSCustomerBranch");
    ...
    {
        ShowModel("Alert", "Please select Company location.")
    ...
    /**Check product price and quantity of products**/
    if (checkPriceandQuantity()) {

        if (parseFloat(txtTotalValue.val()) < 25000 && ddlApprovalStatus.val() == "Final") {
            $("#hdnApprovalStatus").val(ddlApprovalStatus.val());
            $("#hdnTotalValue").val(txtTotalValue.val());
        }
        ...
        var poViewModel = {
            ...
            CustomerBranchId: CustomerBranchId.val(),
        ...
        $.ajax({
            url: "../PO/AddEditPO?AccessMode=" + accessMode + "",  //accessMode for new enhancement by hari sir discussed.
            cache: false,
            type: "POST",
            dataType: "json",
            data: JSON.stringify(requestData),
            ...
            success: function (data) {
                if (data.status == "SUCCESS") {
                    ...
                    if (parseFloat($("#hdnTotalValue").val()) < 25000 && $("#hdnApprovalStatus").val() == "Final") {
                        SendMailOnPOCreate(data.trnId);
                    }
                    ClearFields();
                    setTimeout(
                      function () {
...
function GetVendorDetail(vendorId) {
    $.ajax({
        ...
        success: function (data) {
            ...
            $("#txtBGSTNo").val(data.GSTNo);
            //Set GST Percentage----------
            if ($("#txtVendorName").val() != "" && $("#txtConsigneeName").val() != "") {
...
function GetConsigneeDetail(consigneeId,consigneeBranchId) {
    $.ajax({
        type: "GET",
        asnc: false,
        ...
        success: function (data) {
            ...
            if (consigneeId != null)
            {
                BindConsigneeBranchList(consigneeId, consigneeBranchId);
            }
...
function ExecuteSave() {
    if (SetGSTPercentageInProduct() === true || SetGSTPercentageInProduct() === undefined) {
        setTimeout(
function () {
    SaveData();
}, 1000);
...
function GetPurchaseIndentDetail(indentId) {
    $.ajax({
        ...
        success: function (data) {
            ...
            GetConsigneeDetail(data.CustomerId, data.CustomerBranchId);
...
function SendMailOnPOCreate(POId) {
    var requestData = { poId: POId, reportType: "PDF" };
    $.ajax({
        url: "../PO/POMail",
        cache: false,
        type: "GET",
...
function checkPriceandQuantity()
{
    var flag = true;
    var productPrice = 0;
    $('.prod-price').each(function () {
        ...
        if ($this == "0" || $this == "0.00") {
            ShowModel('Alert','Product Price should be greater than zero');
            flag = false;
            return false;
        }
    });
    ...
    $('.prod-quantity').each(function () {
        $this = parseInt($(this).text());
        if ($this == "0" || $this == "0.00") {
            ShowModel('Alert','Product Quantity should be greater than zero');
            flag = false;
            return false;
        }
    });
...
function ResetPage() {
    if (confirm("Are you sure to reset all fields?")) {
        window.location.href = "../PO/AddEditPO?accessMode=1";
    }
}
